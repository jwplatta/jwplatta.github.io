...
I was constantly moving back and forth between learning, building, and doing the work. Sometimes I discovered things I didn’t understand. For example, one bug effectively turned a supposedly market-neutral portfolio into a long-only strategy. The issue was easy to fix, but the danger was recognizing that the output didn’t make sense. In my experience, LLMs excel at writing running code, but not necessarily writing correct code. Because I understood the research process, I had a clear expectation for how the strategy should behave. That expectation made it obvious when the implementation and the results diverged.
